...
async function sendEmail({ to, subject, bodyHtml, pdfBase64, pdfFilename, threadId, inReplyTo, references, cc, bcc }) {
  ...
  // Applies whenever there is no To at all — Bcc-only, Cc-only, or both. A message with an
  // empty To line reads as malformed to some clients and scores worse with spam filters.
  if (!to && (bcc || cc)) to = await getOwnAddress();
